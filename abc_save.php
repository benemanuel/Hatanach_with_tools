<?PHP
function abc_editor_top ($output,$heading='Test')
{
if (!isset($output))
...
function abc_editor_bottom ($output)
{
if (!isset($output))
{ $output = fopen('php://output', 'w');}
...
function engrave_music($cit_string,$title2,$book,$melody,$hebrew_lyrics,$translit_lyrics)
{
    global $debug_flag;
    $debug_flag=true;
    if ($debug_flag) {
    ...
    if (!empty($cit_string)) //we have what to publish
    {
        ...
        abc_editor_top($myfile,$cit_string);
    fwrite($myfile,'T: '.$cit_string.PHP_EOL);
    ...
    if ((count($m) == count($h)) && (count($h) == count($t)))
    {
        for($i = 0;$i < count($m);$i++)
         {
             fwrite($myfile,rtrim($m[$i]).PHP_EOL);
             fwrite($myfile,'W: '.rtrim($h[$i]).PHP_EOL);
             fwrite($myfile,'W: '.rtrim($t[$i]).PHP_EOL);
         }
    } else
    {
    ...
    fclose($myfile);
    echo  "<a target = '_blank' href='https://hatanach.geulah.org.il/verse/".$output."'/>". $cit_string."</a><br>";
