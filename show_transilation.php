...
function show_transilation($key, $cit, $lang, $version, $display=0, $url="https://www.biblegateway.com/passage/?search="){
    ...
    if ($engcit == "Same"){
        if (strpos($cit,"(") === false) {$engcit = $cit;}
        else  {$engcit = str_replace(")(",",",substr($engcit, 1, -1));}
    }
    if ($debug_flag){ echo PHP_EOL."<p debug='fun show_transilation:"; print_r($engcit); echo "'></p>";}
    if ($lang == "he") {
...
#       	  echo '<p '. $text_style. '>The '. ucfirst($lang). ' version '. $version. ' Translation is:</p>';
          echo '<p '. $text_style. '>The ';
          echo '<a target = '. "'_blank'". ' href="https://www.biblegateway.com/verse/'. strtolower($lang). '/' . $engcit .'"/>';
          echo ucfirst($lang) . '</a>';
          echo ' version '. $version. ' Translation is:</p>';
